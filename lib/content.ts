...
export type PublishedContent = { metadata: ContentMetadata; body: string; sources: KnowledgeSource[] };
...
export function getContent(locale: Locale, section: string, slug: string): PublishedContent | null {
  for (const sectionDir of [...new Set([section, normalizedSection(section)])]) {
    ...
    const knowledge = JSON.parse(fs.readFileSync(path.join(knowledgeRoot, `${metadata.primary_object_id}.json`), "utf8")) as { sources: KnowledgeSource[] };
    return { metadata, body, sources: knowledge.sources.filter((source) => metadata.source_ids.includes(source.id)) };
  }
  return null;
}
