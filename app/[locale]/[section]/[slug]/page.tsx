...
export function generateStaticParams() {
  const releases = listContent().filter((item) => item.content_type !== "encyclopedia" && item.content_type !== "designer_profile" && item.content_type !== "artist_profile").map((item) => ({ locale: item.locale, section: item.content_type === "research" || item.content_type === "case_study" || item.content_type === "visual_analysis" ? "articles" : item.content_type === "collection" ? "collections" : item.content_type, slug: item.slug }));
