...
export default async function PhotographersListingPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const entriesLabel = locale === "ru" ? "Материалы раздела" : "Entries in this section";
  ...
  return (
    <section className="shell page-shell">
      ...
      <div className="listing-header"><p>{releases.length ? entriesLabel : content.labels.comingSoon}</p><span>{(releases.length || 1).toString().padStart(2, "0")}</span></div>
